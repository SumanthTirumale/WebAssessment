from assessment.helpers import get_payloads, change_exclusion_header_format, insert_data, filter_url,\
    print_status_code, print_message
from assessment.helpers import inject_payload
...
class BufferOverflow:
    """Class to bufferoverflow vulnerability attack"""
    def __init__(self, **kwargs):
        ...
        self.payloads = get_payloads(BUFFEROVERFLOW)
        self.header = dict()
                ...
                for key, value in self.header.items():
                    ...
                    if response == 000 or str(response.status_code).startswith("2") \
                        ...
                        if self.active_header_audit:
                            ...
                            self.add_data(
                                status_code=response.status_code, tag="None", actual_data=header, payload=payload,
                                current_data=response_header, result="Vulnerable",
                                act_header_adt_rsp=active_header_response, act_header_adt_rsl=active_header_result
                            )
                        else:
            ...
            for key, value in body_array.items():
                ...
                if response == 000 or str(response.status_code).startswith("2") \
                    ...
                    if self.active_header_audit:
                        ...
                        self.add_data(
                            status_code=response_code, tag=tag, actual_data=current_val, payload=payload,
                            current_data="None", result="Vulnerable", act_header_adt_rsp=active_header_response,
                            act_header_adt_rsl=active_header_result
                        )
                        ...
                        self.add_data(
                            status_code=response_code, tag=tag, actual_data=current_val, payload=payload,
                            current_data="None", result="Vulnerable"
                        )
    ...
    def attack(self):
        ...
        print(colored("######################## BufferOverflow Attack ########################", 'cyan'))
        ...
        for base_request in base_requests['baseRequests']:
            ...
            if len(base_request['bodyArray']) > 0 and len(base_request['bodyRaw']):
                ...
                for payload in self.payloads:
                    ...
                    if self.body_audit:
                        self.body_attack(
                            payload, base_request['bodyRaw'], base_request['headers'], base_request['bodyArray']
                        )
